...
foreach ($language_array as $key => $value) {
    $result = DB_query("SELECT  `string`, `tags` FROM `gl_originals` WHERE `language_array`='{$value->array_name}' AND `array_index`='{$value->array_index}' ");
    ...
    $result = DB_query("SELECT `translation`, `id` FROM `gl_translations` WHERE `language_full_name`='{$language}' AND  `language_array`='{$value->array_name}' AND `array_key`='{$value->array_index}' ORDER BY `approval_counts` DESC LIMIT 1");
    ...
    if ($value->translation_id) {
        $result = DB_query("SELECT `sign` FROM `gl_votes` WHERE `user_id` = {$user_id} AND `translation_id`='{$value->translation_id}'");
