...
function add_form_element( &$form, $count, $value, $base_url, $disabled_up, $disabled_down )
{
    ...
    $form_label             = "<label for='translator_input_{$count}'>{$value->string}</label>" . " <img class='form_image' src='{$remove_highlight_image}' id='translator_input_{$count}_image' onclick=remove_highlight() />" . " <img class='form_image' src='{$highlight_image}' id='translator_input_{$count}_image' onclick=highlight() />";
...
function get_language_array( )
{
    ...
    foreach ( $reference as $key => $value ) {
        ...
        $obj->metadata    = "array_{$array}index_{$index}";
        $obj->string      = '';
...
function purge_language_array( $html, &$language_array )
{
    ...
    $taged_strings = array( );
    foreach ( $language_array as $key => $value ) {
        $query  = "SELECT  `string`, `tags` FROM {$_TABLES['originals']} WHERE `language_array`='{$value->array_name}' AND `array_index`='{$value->array_index}' ";
        $result = DB_query( $query );
        while ( $row = DB_fetchArray( $result ) ) {
        ...
        if ( $is_taged == false ) {
            if ( !empty( $value->string ) ) {
                if ( strpos( $html, $value->string ) == false ) {
                    ...
                    $language_array[ $key ]->parsed = $value->string;
                }
            } else {
                unset( $language_array[ $key ] );
            }
            ...
            if ( $in_html == false ) {
                ...
                $numItems = count( $substrings );
                while ( !isset( $substrings[ $numItems ] ) || empty( $substrings[ $numItems ] ) || $substrings[ $numItems ] == ' ' ) {
                    $numItems--;
                }
                $start                          = strpos( $html, $substrings[ 0 ] );
                $length                         = ( strpos( $html, $substrings[ $numItems ], $start - 1 ) + strlen( $substrings[ $numItems ] ) ) - $start;
                $language_array[ $key ]->parsed = substr( $html, $start, $length );
            }
        }
    }
